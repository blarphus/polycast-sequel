...
struct TodayWordsProvider: TimelineProvider {
    ...
    func getSnapshot(in context: Context, completion: @escaping (TodayWordsEntry) -> Void) {
        let snapshot = context.isPreview ? TodayWordsWidgetSnapshot.sample : TodayWordsWidgetStore.loadSnapshot()
        completion(TodayWordsEntry(date: .now, snapshot: snapshot, state: TodayWordsWidgetStore.loadState()))
    }
