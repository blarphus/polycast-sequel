...
- [ ] **AUD-002 — Correct API trust boundaries, roles, and sessions.** Roles/origins/profile switching/revocable sessions and migration/PG tests are implemented. Exact production web/Worker origins are now declared in Render/Wrangler and machine-checked; web, extension, and iOS perform one centralized, detailed, visible, logged teardown on an authenticated 401. Remaining: production environment verification after deployment.
