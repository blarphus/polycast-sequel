...
- [x] **AUD-002 — Correct API trust boundaries, roles, and sessions.** Roles/profile switching/revocable sessions and migration/PG tests are implemented. HTTP and Socket.IO share one tested exact-origin policy; an extension origin is admitted only when its stable ID is explicitly configured. Render deployment `dep-d99qp88k1i2s73ekbnjg` is live, migrations/startup are green, allowed/hostile CORS and correlation behavior were probed, and a temporary production account proved role-elevation rejection, token rotation, old-token rejection, logout revocation, and cleanup (`deleted: 1`). Web, extension, and iOS also perform one centralized, detailed, visible, logged teardown on an authenticated 401.
...
- [x] Re-run all platform gates and complete a requirement-by-requirement audit against all 24 findings. Twenty-three findings are acceptance-complete; only AUD-001 remains open because the live Worker still rejects the new scoped server token until its credentialed rotation/deployment is authorized.
