...
const words = [
  { id: '1', word: 'abatir', lemma: 'abatir', forms: '["abatido"]', translation: 'to bring down', sentence_context: 'El sospechoso fue abatido.', target_language: 'es' },
...
test('practice exercises cover vocabulary interactions without grammar generation', () => {
  ...
  for (const kind of kinds) {
    const exercise = __test.makeExercise(kind, words[0], words, 'es');
...
test('context blanks accept the saved surface form', () => {
  assert.equal(__test.blankContext(words[0]), 'El sospechoso fue _____.');
  const exercise = __test.makeExercise('context_type', words[0], words, 'es');
