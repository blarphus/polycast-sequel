...
export function sharedEntryToEnrichment(entry) {
  ...
  return {
    ...
    lemma_occurrences_per_billion: entry.lemma_occurrences_per_billion == null ? null : Number(entry.lemma_occurrences_per_billion),
    frequency_confidence: entry.frequency_confidence ?? null,
...
export async function storeSharedEntry(payload, db = pool) {
  ...
  const { rows } = await db.query(
    ...
    [
      ...
      payload.lemma_occurrences_per_billion ?? null,
      payload.frequency_confidence ?? null,
      JSON.stringify(payload.frequency_sources || []),
    ],
  );
  return rows[0] || null;
}
